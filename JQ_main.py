# version 5.0：
# running locally√
# ordered by account number and trading dates in single stock & portfolios spreadsheets
# final output with same metrics as First Capital
# New Function: You can have beginDay to be any previous day you need
...
def getAllRecords(jiaoyipath):
    ...
    # 时间转换为可以比较大小的格式
    allTradingRecords['时间'] = allTradingRecords['时间'].apply(
        lambda x: int(datetime.datetime.strftime(datetime.datetime.strptime(x, "%H:%M:%S"), '%H%M%S')))
...
# 功能：读取前一个工作日收益计算结果中的单票记录
# index：需要读取的列名
# rootpath：运营数据\
# 输出：
#     summary：历史上所有客户所有股票每天的交易记录

    # 找到上一个交易日的收益结果文件
    yuanpath = rootpath + "/" + "收益计算结果"
    # 之后改index要用到这个，现在先用着原来的index
    # if changedate(getLastWeekDay(today) ) <= changedate("2019-08-06"):
    #     index = ['新的index']
    lastWeekDay = getLastWeekDay(today)
    if danOrComb == 'dan':
        path = yuanpath + "/" + lastWeekDay + "/" + "汇总数据" + "/" \
               + lastWeekDay + "_单票收益记录.xlsx"
    if danOrComb == 'comb':
        path = yuanpath + "/" + lastWeekDay + "/" + "汇总数据" + "/" \
               + lastWeekDay + "_组合收益记录.xlsx"
...
def main(data_zu, jiaoyipath, rootpath):
    ...
    #     today = time.strftime('%Y-%m-%d', time.localtime(time.time())) 从外面传进来，这样可以统一改
...
def finalmain(dan_path, zu_path, jiaoyipath, rootpath):
    ...
    official_path = rootpath + "/" + "聚宽云端）正式使用登记表.xlsx"
    yuanpath = rootpath + "/" + "收益计算结果"
    ...
    # 没有当天市值的日期
    print(len(datelist))
    print("日期：从{}到{}".format(datelist[0], datelist[-1]))    # 建立新的dataframe，用以储存所以的数据
...
def findpotentialclient(rootpath, today):
    # 函数功能：定义查找潜在客户名单
    # 杜伦：不知道用来干嘛的, 先不删, 万一以后会用到
...
def getYYBName(account):
    if account == 156062015:
        ...
        department = '泓大资产'
    elif account == 666628887875:
        department = '华泰证券杭州东横街营业部'
    else:
...
def dealBookDate(df, allTradingRecords):
    ...
    for i in range(len(List)):
        ...
        # 如果不在，从交易记录中获取相关的股票列表，但是市值，授权资金都是nan，今天也不会操作
        else:
            cusDitalDict = {}
            cusDitalDict['客户姓名'] = List1[i]
            cusDitalDict['股票列表'] = getstocklist(List[i], allTradingRecords)  # 这里拿的是这个用户历史上做过的左右票的列表，感觉和上面不一致
            cusDitalDict['营业部'] = getYYBName(List[i])
            cusDict[List[i]] = cusDitalDict
    return cusDict
...
def getYongJinRate(df):
    ...
    return round(yongJinRate, 6), isMian5
...
def subProcess(bookDict, index, summary_dan, stockrankdf, allTradingRecords, beginDay):
    ...
    # 更新收益结束，排序然后输出
    allCusOutDf = allCusOutDf.sort_values(by=['营业部', '资金账号', '交易日期'], ascending=(True, True, True))
    allCusOutDf = allCusOutDf[index]
    return allCusOutDf, isMian5Dt


def processACusComADay(df, account, day, trad_dayrecord, yongJinRate, isMian5, index):
...
def processComADay(aDayDanData, allTradingRecords, allCombOutDf, isMian5Dt, day):
    ...
    for account in aDayAccounts:
        ...
        aRecorderDict = processACusComADay(aCusAdayData, account, day, aCusTodayRecord, yongJinRate, isMian5, index)
...
def combProcess(allCusOutDf, bookDict, index, summary_comb, allTradingRecords, isMian5Dt, beginDay):
    ...
    for date in dealDayList:
        ...
        # 更新组合收益结束，排序并输出
    allCombOutDf = allCombOutDf.sort_values(by=['营业部', '资金账号', '交易日期'], ascending=(True, True, True))
...
#获取今天日期
# today = '2019-10-09' #这样就可以随便跑了
today = time.strftime("%Y-%m-%d",time.localtime(time.time()))
bookName = 't0_total_stock_list_'+ today.replace('-', '') +'.csv'
YYBname = '营业部配置表.xlsx'
jiaoyipath = 'trans-2019-10-11-16-04-07'
...
stockrankpath = rootpath + "/" + "Alpha-T个股近1月收益排名_20190925.xlsx"  # 股票池 并非每日更新

beginDay = getLastWeekDay(today)  # 默认从前一个工作日开始跑收益
jiaoYiNameDict = listFilename(jiaoyipath) #jiaoYiNameDict：一个以资金账号为key，文件名为value的dict
...
bookDict = dealBookDate(bookDF, allTradingRecords) # 记录交易记录中所有用户的所有股票，不过只有今天在用户配置表里的才有用户授权资金和市值
...
#findpotentialclient(rootpath, today)
print('#----------------收益表格处理结束--------------------#')
