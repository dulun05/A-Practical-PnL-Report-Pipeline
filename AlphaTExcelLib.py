# -*- coding: utf-8 -*-
"""
Created on Tue Aug 23 13:33:45 2019
...
#修改日期：2019/4/10  增加资金账号 
        ...
        for y in range(leni):
            for i in range(leny):
                ...
                if i == 0:
                    ...
                    rows[i][y].font = font
                    if not pd.isnull(rows[i][y].value) and rows[i][y].value != "账户名":
                        try:    
                            l = len(rows[i][y].value)
                            if l==2:
                                rows[i][y].value = rows[i][y].value[0]+'*'
                            elif l>=2:
                                rows[i][y].value = rows[i][y].value[0]+'*'+rows[i][y].value[-1]
                            else:
                                rows[i][y].value = '*'
                        except:
                            
                            rows[i][y].value = rows[i][y].value
    ...
    def process_main(self,data_dan, data_zu):
        ...
        for each in YYBList:
            ...
            self.mkdir(YYBPath)
            # 将该营业部的文件放入目标目录
            strTemp = self.today +"隐藏"+ '_' + each + '_' + '收益汇总表' + ".xlsx"
